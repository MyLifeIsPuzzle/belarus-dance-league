...
public final class TestDataImporter {
    ...
    public void importTestData(SessionFactory sessionFactory) {
        try (Session session = sessionFactory.openSession()) {
            ...
            DancingHall school = new DancingHall("School 123", new Address("Minsk", "Belskogo 12", "Great hall"), true);
            session.save(school);
            DancingHall cdoimTanka = new DancingHall("CDOIM contact", new Address("Minsk", "M.Tanka 10", "Great hall"), true);
            session.save(cdoimTanka);

            Club blackFox = new Club("Black fox", "some info", true);
            session.save(blackFox);
            Club jungleMove = new Club("Jungle move", "some info", true);
            session.save(jungleMove);

            Coach daniliuk = new Coach("Irina", "Daniliuk", true, "Some info", new HashSet<>(Collections.singletonList(blackFox)));
            session.save(daniliuk);
            Coach akhatova = new Coach("Juliya", "Akhatova", true, "Some info", new HashSet<>(Collections.singletonList(jungleMove)));
            session.save(akhatova);

            Account daniliukAcc = new Account("someemail@gmail.com", "pass", true, daniliuk, coachRole);
            session.save(daniliukAcc);
            Account akhatovaAcc = new Account("email@tut.by", "pass", true, akhatova, coachRole);
            session.save(akhatovaAcc);

            DanceGroup warriors = new DanceGroup("Warriors", blackFox, true);
            session.save(warriors);
            DanceGroup archers = new DanceGroup("Archers", jungleMove, true);
            session.save(archers);

            DanceClass firstClass = new DanceClass(archers, Style.DISCO, akhatova, DayOfWeek.FRIDAY, cdoimTanka, LocalTime.now(), true);
            session.save(firstClass);
            DanceClass secondClass = new DanceClass(warriors, Style.HIP_HOP, daniliuk, DayOfWeek.MONDAY, school, LocalTime.now(), true);
            session.save(secondClass);
            DanceClass thirdClass = new DanceClass(archers, Style.HOUSE, akhatova, DayOfWeek.SATURDAY, cdoimTanka, LocalTime.now(), true);
            session.save(thirdClass);

            Dancer dancerOne = new Dancer("Masha", "Chastnaya", true, LocalDate.now(), AgeCategory.JUNIOR, League.BEGINNER,
                    new HashSet<>(Collections.singletonList(archers)), "123-12-12");
            session.save(dancerOne);
            Dancer dancerTwo = new Dancer("Masha", "Balabanovich", true, LocalDate.now(), AgeCategory.JUNIOR, League.BEGINNER,
                    new HashSet<>(Collections.singletonList(archers)), "123-12-12");
            session.save(dancerTwo);
            Dancer dancerThree = new Dancer("Anton", "Podobed", true, LocalDate.now(), AgeCategory.ADULT, League.FIRST_LEAGUE,
                    new HashSet<>(Arrays.asList(archers, warriors)), "123-12-12");
            session.save(dancerThree);
            Dancer dancerFour = new Dancer("Dmitriy", "Nosevich", true, LocalDate.now(), AgeCategory.ADULT, League.FIRST_LEAGUE,
                    new HashSet<>(Collections.singletonList(warriors)), "123-12-12");
            session.save(dancerFour);
            Dancer dancerFive = new Dancer("Artsiom", "Tretinnikov", true, LocalDate.now(), AgeCategory.ADULT, League.FIRST_LEAGUE,
                    new HashSet<>(Collections.singletonList(warriors)), "123-12-12");
            ...
            session.save(new Rating(dancerTwo, 27, Style.DISCO));
            session.save(new Rating(dancerTwo, 17, Style.DISCO));
            ...
            Request request = new Request("Vladislava", "Lavrukhina", LocalDate.now(), archers, "123-12-12", true);
